...
test.group('Group', (group) => {
  ...
  test('it should remove user from group', async (assert) => {
    ...
    const newPlayer = await UserFactory.merge({ password: plainPassword }).create()
    const response = await supertest(BASE_URL)
      .post('/sessions')
      .send({ email: newPlayer.email, password: plainPassword })
    ...
    await supertest(BASE_URL)
      .delete(`/groups/${group.id}/players/${newPlayer.id}`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200)
  ...
  test('it should not remove the group', async (assert) => {
    const groupPayload = {
      name: 'test',
      description: 'test',
      schedule: 'test',
  ...
  test('it should return 404 when providing an unexisting group for deletation', async (assert) => {
    const { body } = await supertest(BASE_URL)
      .delete(`/groups/123`)
      .set('Authorization', `Bearer ${token}`)
      .send({})
